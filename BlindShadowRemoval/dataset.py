...
class Dataset():
    ...
    def parse_fn_test(self, file):
        ...
        def _parse_function(_lm):
            ...
            _lm_part = _lm.split('/')
            _img = _lm.split('.')[0]+'.png'
            _gt  = '/'.join(_lm_part[0:2]+['gt']+_lm_part[-2:]).split('.')[0]+'.png'
            ...
            # img2
            _lm_part = _lm.split('/')
            _path = glob.glob('/'.join(_lm_part[:-1])+'/*.npy') 
            idx = random.randint(0,len(_path)-1)
            _lm = _path[idx]
            _img = _lm.split('.')[0]+'.png'
            print("Image 2: "+_img)
            img = cv2.cvtColor(cv2.imread(_img), cv2.COLOR_BGR2RGB) / 255.
            ...
            reg_in = generate_offset_map(lm, lm_ref, config.IMG_SIZE)
            reg_out= generate_offset_map(lm_ref, lm, config.IMG_SIZE)
            img2 = np.concatenate([img, uvm, reg_in, reg_out, face], axis=2) # 3,3,1,3,3,1
            ...
            reg_in = generate_offset_map(lm, lm_ref, config.IMG_SIZE)
            reg_out= generate_offset_map(lm_ref, lm, config.IMG_SIZE)
            img3 = np.concatenate([img, uvm, reg_in, reg_out, face], axis=2) # 3,3,1,3,3,1
            ...
            reg_in = generate_offset_map(lm, lm_ref, config.IMG_SIZE)
            reg_out= generate_offset_map(lm_ref, lm, config.IMG_SIZE)
            img4 = np.concatenate([img, uvm, reg_in, reg_out, face], axis=2) # 3,3,1,3,3,1
            ...
            reg_in = generate_offset_map(lm, lm_ref, config.IMG_SIZE)
            reg_out= generate_offset_map(lm_ref, lm, config.IMG_SIZE)
            img5 = np.concatenate([img, uvm, reg_in, reg_out, face], axis=2) # 3,3,1,3,3,1
            ...
            reg_in = generate_offset_map(lm, lm_ref, config.IMG_SIZE)
            reg_out= generate_offset_map(lm_ref, lm, config.IMG_SIZE)
            img6 = np.concatenate([img, uvm, reg_in, reg_out, face], axis=2) # 3,3,1,3,3,1
            ...
            reg_in = generate_offset_map(lm, lm_ref, config.IMG_SIZE)
            reg_out= generate_offset_map(lm_ref, lm, config.IMG_SIZE)
            img7 = np.concatenate([img, uvm, reg_in, reg_out, face], axis=2) # 3,3,1,3,3,1
            ...
            reg_in = generate_offset_map(lm, lm_ref, config.IMG_SIZE)
            reg_out= generate_offset_map(lm_ref, lm, config.IMG_SIZE)
            img8 = np.concatenate([img, uvm, reg_in, reg_out, face], axis=2) # 3,3,1,3,3,1
            ...
            reg_in = generate_offset_map(lm, lm_ref, config.IMG_SIZE)
            reg_out= generate_offset_map(lm_ref, lm, config.IMG_SIZE)
            img9 = np.concatenate([img, uvm, reg_in, reg_out, face], axis=2) # 3,3,1,3,3,1
            ...
            reg_in = generate_offset_map(lm, lm_ref, config.IMG_SIZE)
            reg_out= generate_offset_map(lm_ref, lm, config.IMG_SIZE)
            img10 = np.concatenate([img, uvm, reg_in, reg_out, face], axis=2) # 3,3,1,3,3,1
        ...
        _img, _box, _name = tf.numpy_function(_parse_function, [file], [tf.float32, tf.float32, tf.string])
        _img = tf.ensure_shape(_img, [10, config.IMG_SIZE, config.IMG_SIZE, 16])
        _box = tf.ensure_shape(_box, [4])
        return _img, _box, _name
    ...
    def parse_fn_test_FFHQ(self, file):
        print("Here")
        config = self.config
        def _parse_function(_lm):
            _lm = _lm.decode('UTF-8')
